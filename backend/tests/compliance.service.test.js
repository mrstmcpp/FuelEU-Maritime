...
const compliance_service_1 = require("../src/core/application/services/compliance.service");
...
// 🧩 Mock Repository
class MockShipComplianceRepository {
    constructor(initial = []) {
...
describe("ComplianceService", () => {
    ...
    beforeEach(() => {
        ...
        service = new compliance_service_1.ComplianceService(repo);
    });
    ...
    it("should compute CB correctly based on formula", async () => {
        ...
        const energyInScope = fuelTons * constants_1.CONSTANTS.ENERGY_FACTOR_MJ_PER_TON;
        const expectedCb = (constants_1.CONSTANTS.TARGET_INTENSITY_GCO2E_PER_MJ - actualIntensity) *
            energyInScope;
    ...
    it("should throw error if fuelConsumptionTons <= 0", async () => {
        await expect(service.computeCB(1, 2025, 0, 80)).rejects.toThrow("Fuel consumption must be greater than zero.");
    });
    it("should throw error if actualIntensity <= 0", async () => {
        await expect(service.computeCB(1, 2025, 10, 0)).rejects.toThrow("Actual intensity must be greater than zero.");
    });
    ...
    it("should throw error if adjustCB target record not found", async () => {
        await expect(service.adjustCB(999, 2025, 100)).rejects.toThrow("Compliance record not found.");
    });
});
//# sourceMappingURL=compliance.service.test.js.map